...
public class ConditionalGeneratorFactory implements DynamicGeneratorFactory{
  ...
  @Override
  public <D> Optional<GenerationStrategy<D>> buildGeneratorOf(DataKey<D> key, NewFixtureFactory fixture,
      DynamicGeneratorFactory dynamicGeneratorFactory) {

    Function<Fixture,D> g = (Function)generator;

    return spec.apply(key.getType()) ? Optional.of(new SimpleGenerationStrategy<>(key, g,f -> 1)) : Optional.absent();
  }
