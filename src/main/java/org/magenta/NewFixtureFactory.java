...
public class NewFixtureFactory implements Fixture {

  private NewFixtureFactory parent;
  ...
  protected NewFixtureFactory(NewFixtureFactory parent, GenerationStrategyFactory generationStrategyBuilder, DynamicGeneratorFactory generatorFactory,
  ...
  @Deprecated
  protected NewFixtureFactory(org.magenta.NewFixtureFactory prototype) {
    this.parent = prototype.parent;
    this.generationStrategyFactory = prototype.generationStrategyFactory;
    this.context = prototype.context;
    this.registry = new DataSetFunctionRegistry();
    this.dynamicGeneratorFactory = prototype.dynamicGeneratorFactory;
  }

  public NewFixtureFactory newChild() {
    NewFixtureFactory child = new NewFixtureFactory(this, generationStrategyFactory, dynamicGeneratorFactory, context);
    return child;
  }

  @Deprecated
  public <D> DataSet<D> generator(Class<D> type) {
    return dataset(type);
  }
  ...
  @Override
  public NewFixtureFactory init(Class<?> type) {
  ...
  @Override
  public NewFixtureFactory init(DataKey key) {
  ...
  @Override
  public NewFixtureFactory init(Class<?> type, int size) {
  ...
  @Override
  public NewFixtureFactory init(DataKey key, int size) {
  ...
  public <D> DataSetBuilder<D,D> newDataSet(DataKey<D> key) {
    return new DataSetBuilder<>(key, Functions.identity());
  }

  public <D> DataSetBuilder<D,D> newDataSet(Class<D> key) {
    return new DataSetBuilder<>(DataKey.of(key), Functions.identity());
  }

  public <D> DataSetBuilder<D,D> newDataSet(Class<D> key, String qualifier) {
    return new DataSetBuilder<>(DataKey.of(qualifier, key), Functions.identity());
  }
  ...
  public <D> GeneratorBuilder<D> newGenerator(DataKey<D> key) {
    ...
    return new GeneratorBuilder<>(key);
  }

  public <D> GeneratorBuilder<D> newGenerator(Class<D> key) {

    return new GeneratorBuilder<>(DataKey.of(key));
  }

  public <D> GeneratorBuilder<D> newGenerator(Class<D> key, String qualifier) {

    return new GeneratorBuilder<>(DataKey.of(qualifier, key));
  }

  @Override
  public NewFixtureFactory restrictTo(Object first, Object... theRest) {

    NewFixtureFactory child = newChild();
  ...
  public class DataSetBuilder<D,S> {
    ...
    public <NEW_TYPE>  DataSetBuilder<D,NEW_TYPE> transformed(Function<NEW_TYPE,S> converter) {
      ...
      return new DataSetBuilder<>(key, newFunction);
    }
    ...
    private void composedOf(ImmutableList<S> items) {
      DataSet<D> dataset = new DataSetImpl<>(new TransformedDataSupplierDecorator<>(new StaticDataSupplier<>(items,  (TypeToken<S>)this.key.getType()),transform,key.getType()), true);
    ...
    private Function<Fixture, DataSupplier<D>> toDataSupplier(final GenerationStrategy<S> strategy, Optional<Integer> numberOfItems) {
      return fixture ->
      new TransformedDataSupplierDecorator<>(
    ...
    private Function<Fixture, DataSupplier<D>> toDataSupplier(final GenerationStrategy<? extends Iterable<S>> strategy) {
      return fixture -> new TransformedDataSupplierDecorator<>(
          new LazyGeneratedCollectionDataSupplier<>(() -> strategy.generate(fixture),  (TypeToken<S>)this.key.getType()), transform, key.getType());
    }
  }
  ...
  public class GeneratorBuilder<D> {
    ...
    public void generatedBy(final Supplier<? extends D> generator) {
      generatedBy(generator, Optional.<Integer> absent());
    }

    public void generatedBy(Supplier<? extends D> generator, Integer defaultSize) {
    ...
    private void generatedBy(Supplier<? extends D> generator, Optional<Integer> numberOfItems) {

      checkNotNull(generator, "generator argument is null");

      Function<Fixture, DataSet<D>> getDataset = cache(toDataSet(toDataSupplier(toGenerationStrategy(key, (Supplier<D>)generator), numberOfItems)));
  ...
  private <X> GenerationStrategy<X> buildDynamicGenerator(DataKey<X> type) {
    return dynamicGeneratorFactory.buildGeneratorOf(type, NewFixtureFactory.this, dynamicGeneratorFactory).get();
  }
